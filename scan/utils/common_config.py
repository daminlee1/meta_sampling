...
from utils.collate import collate_custom, scan_collate
...
def get_model(p, pretrain_path=None):
    # Get backbone
    if p['backbone'] == 'resnet18':
        if p['train_db_name'] in ['cifar-10', 'cifar-20', 'svkpi', 'svkpi_v2']:
        ...
        elif 'svkpi_v2' in p['train_db_name']:
            from models.resnet import resnet50
            backbone = resnet50() 
    ...
    #pretrain_path = None #prevent assert error on line94
...
def get_train_dataset(p, transform, to_augmented_dataset=False,
    ...
    elif p['train_db_name'] == 'svkpi_v2':
        from data.svkpi import SVKPI_V2
        dataset = SVKPI_V2(train=True, transform=transform, download=False, to_neighbors_dataset = to_neighbors_dataset)
    else:
...
def get_val_dataset(p, transform=None, to_neighbors_dataset=False, to_matching_dataset=False):
    # Base dataset
    if p['val_db_name'] == 'cifar-10':
        ...
        from data.svkpi import SVKPI_V2
        dataset = SVKPI_V2(train=False, transform=transform, download=False, to_neighbors_dataset=to_neighbors_dataset)
    ...
    elif to_matching_dataset:
        from data.custom_dataset import MatchingDataset
        dataset = MatchingDataset(dataset)

    return dataset


def get_train_dataloader(p, dataset, _collate_fn = 'moco'):
    if _collate_fn == 'cluster':
        collate_fn = scan_collate
    elif _collate_fn == 'moco':
        collate_fn = collate_custom
    return torch.utils.data.DataLoader(dataset, num_workers=p['num_workers'], 
            batch_size=p['batch_size'], pin_memory=True, collate_fn=collate_fn,
            drop_last=True, shuffle=True)


def get_val_dataloader(p, dataset, _collate_fn = 'moco'):
    if _collate_fn == 'cluster':
        collate_fn = scan_collate
    elif _collate_fn == 'moco':
        collate_fn = collate_custom
    return torch.utils.data.DataLoader(dataset, num_workers=p['num_workers'],
            batch_size=p['batch_size'], pin_memory=False, collate_fn=collate_fn,
