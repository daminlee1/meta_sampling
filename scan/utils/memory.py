...
class MemoryBank(object):
    ...
    def mine_nearest_neighbors_of_each_images(self, topk, train_memory_bank, calculate_accuracy=True):
        # mine the topk nearest neighbors for every sample
        import faiss
        
        features_target = self.features.cpu().numpy()
        features_train = train_memory_bank.features.cpu().numpy()
        n, dim = features_train.shape[0] + 1, features_train.shape[1]
        #print("n :",n, " dim : ", dim)
        index = faiss.IndexFlatIP(dim)
        index = faiss.index_cpu_to_all_gpus(index)
        ...
        for i in range(features_target.shape[0]):
            each_target = np.array([features_target[i]])
            distances, idx = index.search(each_target, topk+1) # Sample itself is included
            new_idx = [i]
            for j in idx[0]:
                new_idx.append(j)
