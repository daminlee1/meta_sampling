...
@torch.no_grad()
def get_predictions(p, dataloader, model, return_features=False, return_names=False):
    ...
    for iter, batch in enumerate(dataloader):
        if iter % 25 == 0:
            print("eval iter {}/{} latency : {}".format(iter,len(dataloader), (time.time() - start)/25))
            start = time.time()
            
        anchors_img, neighbors_img, neighbors_list, target, anchor_names = batch
        
        # img_show = inv_normalize(batch[key_][0].detach()).numpy()*255
        # img_show = np.array(np.transpose(img_show,(1,2,0)), dtype=np.uint8)
        # print(img_show.shape)
        # img_s = Image.fromarray(img_show)
        # plt.imshow(img_s)
        # plt.show()     
        images = anchors_img.cuda(non_blocking=True)
        bs = images.shape[0]
        res = model(images, forward_pass='default')
        output = res
        if return_features:
            output = res['output']
    ...
    if include_neighbors:
        ...
        if return_names:
            out = [{'predictions': pred_, 'probabilities': prob_, 'targets': targets, 'neighbors': neighbors, 'img_names':img_names} for pred_, prob_ in zip(predictions, probs)]
    else:
...
@torch.no_grad()
def get_lowesthead_predictions(p, dataloader, model, return_features=False, return_names=False):
    # Make predictions on a dataset with neighbors
    model.eval()
    predictions = [[] for _ in range(p['num_heads'])]
    probs = [[] for _ in range(p['num_heads'])]
    ...
    for iter, batch in enumerate(dataloader):
        if iter == 2:
            break
        if iter % 25 == 0:
            print("eval iter {}/{}".format(iter,len(dataloader)))
...
def scan_evaluate(predictions):
    ...
    for head in predictions:
        ...
        #anchor and neighbors similarity
        #print(probs.shape, anchors.shape, neighbors.shape, neighbors.contiguous().view(-1).shape)

        #anchors_prob = probs[anchors]
