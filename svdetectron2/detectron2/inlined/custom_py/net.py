...
import detectron.utils.enc as enc_utils
...
def initialize_gpu_from_weights_file(model, weights_file, gpu_id=0, key=None):
    ...
    weights_data = open(weights_file, 'rb').read()
    if key is not None:
      weights_data = enc_utils.aes_decrypt(key, weights_data)
    src_blobs = pickle.loads(weights_data)
    if 'cfg' in src_blobs:
        ...
        for unscoped_param_name in unscoped_param_names.keys():
            ...
            if dst_name in ws_blobs:
                ...
                if ws_blob.shape != src_blobs[src_name].shape:
                    print('Workspace blob {} with shape {} does not match '
