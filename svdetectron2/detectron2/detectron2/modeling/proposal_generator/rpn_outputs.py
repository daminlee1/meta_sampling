...
def find_top_rpn_proposals_with_gt(
    proposals,
    ...
    for n, image_size in enumerate(image_sizes):
        ...
        gt_boxes = [x["gt_bbox"] for x in gt_anno[n]]
        if len(gt_boxes) == 0:
            gt_boxes = [[0.0, 0.0, 0.0, 0.0]]
        gt_boxes = Boxes(gt_boxes).to(device)
        

        pboxes = boxes[keep]
        match_quality_matrix = retry_if_cuda_oom(pairwise_iou)(gt_boxes, pboxes)        
        maxValue, midx_gt = torch.max(match_quality_matrix, 0)

        new_keep = ((maxValue>=iou_minmax[0]) & (maxValue < iou_minmax[1])).nonzero()
        new_keep = torch.flatten(new_keep)
  
        # # filltering proposals
        # new_keep = []
        # for i in range(len(keep)):            
        #     pbox = boxes.tensor[keep[i]]
        #     for gt in gt_anno[n]:               
        #         # calculate iou
        #         iou = get_iou(pbox.cpu().numpy(), gt["gt_bbox"])
        #         if iou >= iou_minmax[0] and iou < iou_minmax[1]:
        #             # new_keep.append(keep[i].cpu().numpy().tolist())
        #             new_keep += [keep[i].cpu().numpy().tolist()]
        #             break
       
        # new_keep = torch.tensor(new_keep, device=device)
        
        res = Instances(image_size)
        # res.proposal_boxes = boxes[keep]
        # res.objectness_logits = scores_per_img[keep]

        res.proposal_boxes = pboxes[new_keep]
        n_scores_per_img = scores_per_img[keep]
        res.objectness_logits = n_scores_per_img[new_keep]
...
class RPNOutputs(object):
    def __init__(
        ...
        # self.multilevel_rpn = multilevel_rpn
        # if self.multilevel_rpn:
        #     self.num_rpns = num_rpns
        #     self.pred_objectness_logits_set = []
        #     self.pred_anchor_deltas_set = []
        #     for i in range(0, num_rpns):
        #         self.pred_objectness_logits_set.append([
        #             # Reshape: (N, A, Hi, Wi) -> (N, Hi, Wi, A) -> (N, Hi*Wi*A)
        #             score.permute(0, 2, 3, 1).flatten(1)
        #             for score in pred_objectness_logits[i]
        #         ])
        #         self.pred_anchor_deltas_set.append([
        #             # Reshape: (N, A*B, Hi, Wi) -> (N, A, B, Hi, Wi) -> (N, Hi, Wi, A, B)
        #             #          -> (N, Hi*Wi*A, B)
        #             x.view(x.shape[0], -1, B, x.shape[-2], x.shape[-1])
        #             .permute(0, 3, 4, 1, 2)
        #             .flatten(1, -2)
        #             for x in pred_anchor_deltas[i]
        #         ])
    ...
    def losses(self):
        ...
        # if self.multilevel_rpn:
        #     objectness_loss_set = []
        #     localization_loss_set = []

        #     for i in range(0, self.num_rpns):
        #         objectness_loss, localization_loss = rpn_losses(
        #             gt_labels,
        #             gt_anchor_deltas,
        #             # concat on the Hi*Wi*A dimension
        #             cat(self.pred_objectness_logits_set[i], dim=1),
        #             cat(self.pred_anchor_deltas_set[i], dim=1),
        #             self.smooth_l1_beta,
        #         )
        #         objectness_loss_set.append(objectness_loss)
        #         localization_loss_set.append(localization_loss)
        #     normalizer = self.batch_size_per_image * self.num_images
            
        #     loss_dict = dict()
        #     for i in range(0, self.num_rpns):
        #         rpn_cls_key_str = "loss_rpn_cls" + str(i)
        #         rpn_loc_key_str = "loss_rpn_loc" + str(i)
        #         loss_dict[rpn_cls_key_str] = objectness_loss_set[i] / normalizer
        #         loss_dict[rpn_loc_key_str] = localization_loss_set[i] / normalizer
        #     return loss_dict
