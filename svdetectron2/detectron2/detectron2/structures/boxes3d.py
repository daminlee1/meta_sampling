...
class Boxes3D:
    ...
    def __init__(self, tensor: torch.Tensor):
        """
        Args:
            tensor (Tensor[float]): a Nx4 matrix.  Each row is (x1, y1, x2, y2, x3, y3, x4, y4, x5, y5, x6, y6, x7, y7).
        """
        ...
        if tensor.numel() == 0:
            ...
            tensor = tensor.reshape((0, 14)).to(dtype=torch.float32, device=device)
        assert tensor.dim() == 2 and tensor.size(-1) == 14, tensor.size()

        self.tensor = tensor

    def clone(self) -> "Boxes3D":
        """
        Clone the Boxes3D.
    ...
    def to(self, device: str) -> "Boxes3D":
        return Boxes3D(self.tensor.to(device))
    ...
    def infinite_check(self) -> torch.Tensor:
        return torch.isfinite(self.tensor)
    ...
    def __getitem__(self, item: Union[int, slice, torch.BoolTensor]) -> "Boxes3D":
        """
        Returns:
    ...
    def cat(cls, boxes_list: List["Boxes3D"]) -> "Boxes3D":
        ...
        Arguments:
            boxes_list (list[Boxes3D])

        Returns:
            Boxes3D: the concatenated Boxes3D
        """
