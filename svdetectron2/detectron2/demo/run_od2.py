        ...
        for idx, batch in enumerate(batch_list):            
            ...
            for i in range(len(result[0])):
                ...
                if args.save_outimg:
                    ...
                    det_errs = match_gt_detection(inputs[i]['annotations'], result[0][i])
                    ...
                    if 0:
                        ...
                        for idx in det_errs['miscls_obj']:
                            ...
                            cv2.rectangle(image, (np.int(bbox[0]), np.int(bbox[1])), (np.int(bbox[2]), np.int(bbox[3])), get_color(gtcls), 1)                            
                        cv2.imwrite(outimgfilepath, image)
